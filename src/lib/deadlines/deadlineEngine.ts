...
export async function syncUserDeadlines(userId: string) {
  ...
  // Combine unique scholarships
  const scholarshipMap = new Map();
  
  savedScholarships.forEach(s => {
    scholarshipMap.set(s.scholarshipId, { ...s.scholarship, isSaved: true, matchScore: 100 });
  });

  matches.forEach(m => {
    if (!scholarshipMap.has(m.scholarshipId)) {
  ...
  for (const sch of uniqueScholarships) {
    // If no deadlines exist in relation but there is a main deadline field, create one
    if (sch.deadlines.length === 0 && sch.deadline) {
      try {
        const defaultDeadline = await prisma.scholarshipDeadline.create({
          data: {
        ...
        sch.deadlines.push(defaultDeadline);
      } catch (err) {
        console.error("Failed to create default deadline for", sch.title, err);
      }
    }
