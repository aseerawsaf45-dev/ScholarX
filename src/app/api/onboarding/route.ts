  ...
  try {
    ...
    if (step === 1) {
      await prisma.studentProfile.upsert({
        ...
        update: {
          ...
          undergraduateCgpa: data.undergraduateCgpa ? parseFloat(data.undergraduateCgpa) : null,
    ...
    if (step === 7) {
      await prisma.studentProfile.upsert({
        where: { userId: user.id },
        update: {
          sscGpa: data.sscGpa ? parseFloat(data.sscGpa) : null,
          undergraduateCgpa: data.undergraduateCgpa ? parseFloat(data.undergraduateCgpa) : null,
          graduateCgpa: data.graduateCgpa ? parseFloat(data.graduateCgpa) : null,
          currentSemester: data.currentSemester || null,
          researchExperience: data.researchExperience || null,
          ...
          programmingSkills: data.programmingSkills || null,
          awards: data.awards || null,
          preferredUniversities: data.preferredUniversities || null,
          specialCircumstances: data.specialCircumstances || null,
        },
        create: {
          userId: user.id,
          sscGpa: data.sscGpa ? parseFloat(data.sscGpa) : null,
      ...
      // Sync roadmap and deadlines immediately when onboarding completes
      try {
        const { syncRoadmapToDB } = await import("@/lib/roadmap/generator");
        const { syncUserDeadlines } = await import("@/lib/deadlines/deadlineEngine");
        await Promise.allSettled([
          syncRoadmapToDB(user.id),
          syncUserDeadlines(user.id)
        ]);
      } catch (e) {
        console.error("Auto sync on step 7 complete error:", e);
      }
    }
