...
export function RoadmapContainer() {
  ...
  const filteredTasks = selectedScholarship === "all"
    ? tasks
    : tasks.filter(t => t.linkedScholarships?.includes(selectedScholarship));

  const highPriority = filteredTasks.filter(t => t.status !== "COMPLETED" && t.priorityScore >= 80);
  const upNext = filteredTasks.filter(t => t.status !== "COMPLETED" && t.priorityScore < 80);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card p-4 rounded-xl border border-border">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{filteredTasks.length} total tasks • {completed.length} completed</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <select 
              ...
              className="bg-muted text-foreground border border-border/60 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-primary font-medium"
            >
          ...
          <button
            onClick={() => generateRoadmap()}
            disabled={isGenerating}
            className="text-xs font-medium bg-muted hover:bg-muted/80 text-foreground px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 border border-border/40 ml-auto sm:ml-0"
          >
            {isGenerating ? <Icon name="loader" className="w-3.5 h-3.5 animate-spin" /> : <Icon name="refresh-cw" className="w-3.5 h-3.5" />}
            Refresh Plan
          </button>
        </div>
      </div>
