      ...
      <CardContent className="flex-1 overflow-y-auto pr-2 pt-2 space-y-3">
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div 
              key={task.id} 
              className={`flex items-start space-x-3 p-2 rounded-md transition-colors ${task.status === "COMPLETED" ? "opacity-60" : "hover:bg-muted/50"}`}
            >
              <Checkbox 
                id={task.id} 
                checked={task.status === "COMPLETED"} 
                onCheckedChange={(checked) => toggleTask({ taskId: task.id, status: checked ? "COMPLETED" : "NOT_STARTED" })}
                className="mt-0.5"
              />
              <div className="grid gap-1.5 leading-none flex-1 min-w-0">
                <label 
                  htmlFor={task.id} 
                  className={`text-sm font-medium leading-tight cursor-pointer ${task.status === "COMPLETED" ? "line-through text-muted-foreground" : ""}`}
                >
                  {task.title}
                </label>
              </div>
