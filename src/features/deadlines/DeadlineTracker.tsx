...
function LiveCountdown({ targetDate, isUrgent }: { targetDate: string; isUrgent: boolean }) {
  ...
  if (!timeLeft) return <span className="text-muted-foreground/60">Calculating...</span>;

  const isCritical = timeLeft.d === 0 && timeLeft.h < 24;

  return (
    <div className={`flex gap-2 font-mono text-sm ${isUrgent ? 'text-red-400' : 'text-foreground/80'}`}>
...
export function DeadlineTracker() {
  ...
  const getUrgencyColor = (level: string) => {
    switch (level) {
      ...
      case "MISSED": return "bg-muted border-border text-muted-foreground/60";
  ...
  const filtered = deadlines.filter(d => {
    ...
    if (selectedScholarship === "all") return true;
    return d.scholarshipId === selectedScholarship;
  });
    ...
    <div className="bg-card rounded-2xl border border-border overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-muted/30">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Smart sorting based on urgency & match score</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select 
            ...
            className="bg-muted text-foreground border border-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-primary font-medium"
          >
          ...
          <button 
            ...
            disabled={isSyncing}
            className="p-1.5 bg-muted text-foreground hover:text-primary rounded-lg transition-colors disabled:opacity-50 border border-border/40"
          >
      ...
      <div className="p-4 flex-1 overflow-y-auto min-h-[300px]">
        {deadlines.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground space-y-3 py-8">
            ...
            <button onClick={() => syncDeadlines()} className="text-primary text-sm font-medium hover:underline">
          ...
          <div className="space-y-3">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <Link href={`/scholarships/${item.scholarship.slug}`} className="font-semibold text-foreground hover:text-primary transition-colors block mb-1">
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    <div className="h-1.5 flex-1 bg-muted rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full transition-all duration-1000 ${item.progressPercent === 100 ? 'bg-emerald-500' : 'bg-primary'}`}
            ...
            {filtered.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-4">No deadlines match the current filter.</p>
            )}
          </div>
        )}
      </div>
